...
class Folder {
  ...
  parse(files) {
    ...
    self
      .tree
      .deleteChildItems(self.tree.rootId);
    // 1. 分解当前路径
    let curPath = '';
    path
      .split('/')
    ...
    // 3. 解析缓存为树形菜单虚拟对象 /var/www/html 根据/分割为数组，循环，相加，增加到虚拟缓存
    let vscache = {};
    let parseObj = (o, p) => {
  ...
  setTitle(num) {
    this
      .cell
      .setText(`<i class="fa fa-folder-o"></i> ${LANG['title']} (${num})`);
  }
