...
class Files {
  ...
  // 需要参数 1.cell: 左侧layout.cell对象 2.manager 主对象（index.jsx）
  constructor(cell, manager) {
    ...
    this.reloadToolbar();
    // reloadToolbar(); this.reloadToolbar = reloadToolbar; toolbar点击事件
    ...
    // 双击文件 :如果可预览并且小于 1MB，则进行预览 :如果size < 100kb，则进行编辑，否则进行下载
  ...
  // 刷新当前目录 如果传递路径参数，则刷新该路径下的文件，不跳转，否则刷新&&跳转
  refreshPath(p) {
    ...
    this
      .manager
      .cache
      .del('filemanager-files-' + Buffer.from(path).toString('base64'));
    // 删除文件夹缓存
  ...
  parse(files) {
    ...
    this
      .cell
      .setText(`<i class="fa fa-file-o"></i> ${LANG['title']} (${data.length})`);
    this
      .grid
      .clearAll();
    this
  ...
  fileIcon(name) {
    ...
    const _icons = {
      ...
      'php,asp,jsp,sql,cfm,aspx,html,js,py,rb,pl,go,css,less,jsx,xml,sh,bat,h,cpp,c': 'file-code-o'
    };
    for (let _ in _icons) {
      let _arr = _.split(',');
      _arr.map((a) => {
